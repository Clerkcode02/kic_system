...
return [
     ...
     * ],
     */
    // Most authenticated routes go through the 'api.protected' group
    // (auth:sanctum + verified.account + not-suspended — see
    // bootstrap/app.php); the pre-verification auth/* routes (e.g.
    // GET /auth/me) use a literal 'auth:sanctum' instead. Neither matches
    // the strategy's default 'auth'/'auth:*' pattern alone, so both are
    // listed explicitly — otherwise every route is (wrongly) documented
    // as public.
    'security_strategy' => [
        \Dedoc\Scramble\SecurityDocumentation\MiddlewareAuthSecurityStrategy::class,
        [
            'middleware' => ['api.protected', 'auth:sanctum', 'auth:*'],
            'scheme' => \Dedoc\Scramble\Support\Generator\SecurityScheme::http('bearer'),
        ],
    ],
];
