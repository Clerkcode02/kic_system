...
export function ProviderBookingListPage() {
  const [status, setStatus] = useState<BookingStatus | ''>('')
  const { data, isLoading, isError, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useInfiniteProviderBookings(status || undefined)
  ...
  return (
    <div className="flex flex-col gap-4 p-4 sm:p-6">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold text-gray-900">Booking queue</h1>
        <Select
      ...
      {!isLoading && !isError && bookings.length === 0 && (
        <EmptyState
          ...
          description="Bookings will show up here once customers request your services."
        />
      )}

      <div className="flex flex-col gap-3">
        {bookings.map((booking) => (
          <Link key={booking.id} to={`/provider/bookings/${booking.id}`}>
